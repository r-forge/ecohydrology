...
function (input = matrix(ncol = 2, nrow = 2), streamflow = input[, 
    ...
    if (!is.null(precip))  {
		...
		r <- max(ceiling(-log10(max(na.omit(precip[begin:endindex])))),0)
        axis(side = 4, at = seq(0, round(max(na.omit(precip[begin:endindex])) + 
            10^(-r), r), length = (1 + ifelse(round(max(na.omit(precip[begin:endindex])) + 
            10^(-r), r) < 10*10^(-r), floor(max(na.omit(precip[begin:endindex]))*10^r + 
            1), 4))), labels = seq(0, round(max(na.omit(precip[begin:endindex])) + 
            10^(-r), r), length = (1 + ifelse(round(max(na.omit(precip[begin:endindex])) + 
